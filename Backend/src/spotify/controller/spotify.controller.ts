...
export class SpotifyController
{
	...
	@Put("seek")
	async seekToPosition(@Body() body: {
		deviceId: string;
		progress: number;
		accessToken: string;
		refreshToken: string
	...
	}): Promise<any>
	{
		...
		const { uri, device_id, accessToken, refreshToken } = body;
	...
	{
		const { artistName, trackName, accessToken, refreshToken } = body;

		if (!artistName || !trackName || !accessToken || !refreshToken)
		{
